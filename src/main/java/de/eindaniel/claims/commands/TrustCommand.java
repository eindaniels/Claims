...
public class TrustCommand extends Command {
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull String @NotNull [] args) {
        ...
        if (targetId == null) { p.sendMessage(Claims.getPrefix().append(MiniMessage.miniMessage().deserialize("<#ff1717>Ungültige UUID für Spieler."))); return true; }

        if (c.getTrusted().contains(targetId)) {
            p.sendMessage(Claims.getPrefix().append(MiniMessage.miniMessage().deserialize("<#ff1717>" + target.getName() + " ist bereits getrusted.")));
            return true;
        }

        c.addTrusted(targetId);
        plugin.claimConfig().addTrusted(c, targetId);
