...
public class UntrustCommand extends Command {
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull String @NotNull [] args) {
        ...
        if (!c.getTrusted().contains(targetId)) {
            p.sendMessage(Claims.getPrefix().append(MiniMessage.miniMessage().deserialize("<#ff1717>" + target.getName() + " ist nicht getrusted.")));
            return true;
        }
        c.removeTrusted(targetId);
        plugin.claimConfig().removeTrusted(c, targetId);
